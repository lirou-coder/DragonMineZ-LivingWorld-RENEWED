...
public final class ResetNpcDataConfirmScreen extends Screen implements LivingWorldScreenMarker {
    ...
    public ResetNpcDataConfirmScreen(Screen parent) { super(LWLang.text("screen.reset_npc_data.title")); this.parent = parent; }
    @Override public void render(GuiGraphics g, int mx, int my, float partial) {
        renderBackground(g); int w=500,h=190,x=(width-w)/2,y=(height-h)/2;
        LivingWorldGuiStyle.drawPanel(g,x,y,w,h);
        g.drawCenteredString(font,LWLang.text("screen.reset_npc_data.title"),width/2,y+16,0xFFFF5555);
        String text=LWLang.string("screen.reset_npc_data.body");
        ...
        LivingWorldGuiStyle.drawButton(g,font,x+110,y+h-34,110,20,LWLang.string("button.cancel"),mx,my,true,false,false);
        LivingWorldGuiStyle.drawButton(g,font,x+w-220,y+h-34,110,20,LWLang.string("button.reset"),mx,my,true,false,false);
    }
