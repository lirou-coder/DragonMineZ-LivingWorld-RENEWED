...
public final class FactionTravelScreen extends Screen implements LivingWorldScreenMarker {
    ...
    private FactionTravelScreen(FactionTravelScreenPacket data) {
        super(LWLang.text("screen.faction_travel.title", "Quest Navigation")); this.data = data; this.selected = data.contacts().isEmpty() ? -1 : 0;
    }
    ...
    @Override public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        LivingWorldGuiStyle.drawFitted(graphics, font, LWLang.string("screen.faction_travel.heading", "GET TO QUEST AREA — %s", data.factionName()), left + 18, top + 14, panelW - 72, LivingWorldGuiStyle.GOLD);
        ...
        LivingWorldGuiStyle.drawFitted(graphics, font, LWLang.string("screen.faction_travel.instant_status", "Instant Transmission shortcut: %s", data.instantTransmissionStatus()), left + 18, top + 51, panelW - 36,
                "READY".equals(data.instantTransmissionStatus()) ? LivingWorldGuiStyle.GREEN : LivingWorldGuiStyle.MUTED);
        LivingWorldGuiStyle.drawFitted(graphics, font, LWLang.string("screen.faction_travel.help", "Follow the live quest compass normally, or jump to a trusted contact (REL 60+) in the objective faction."), left + 18, top + 66, panelW - 36, LivingWorldGuiStyle.MUTED);
        ...
        if (data.contacts().isEmpty()) {
            ...
            LivingWorldGuiStyle.drawFitted(graphics, font, LWLang.string("screen.faction_travel.no_contact", "No trusted contact near this objective yet."), left + 30, y + 12, panelW - 60, LivingWorldGuiStyle.TEXT);
            LivingWorldGuiStyle.drawFitted(graphics, font, LWLang.string("screen.faction_travel.no_contact_help", "Use the live compass, or build a real relationship with one of their members to unlock this shortcut."), left + 30, y + 29, panelW - 60, LivingWorldGuiStyle.MUTED);
            ...
            String meta = LWLang.label("faction_role", c.role()) + " • " + LWLang.label("fighter_rank", c.rank())
                    + (c.activity().isBlank() ? "" : " • " + LWLang.label("activity", c.activity()));
        ...
        LivingWorldGuiStyle.drawButton(graphics, font, left + 18, top + panelH - 43, 166, 24, LWLang.string("screen.faction_travel.transmit", "Transmit to Contact"), mouseX, mouseY, canTravel, false, true);
        LivingWorldGuiStyle.drawButton(graphics, font, left + panelW - 168, top + panelH - 43, 150, 24, LWLang.string("screen.faction_travel.back", "Back to Quest"), mouseX, mouseY, true, false, false);
