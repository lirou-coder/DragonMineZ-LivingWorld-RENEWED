...
public final class LWSagasEffectsRenderer extends GeoEntityRenderer<AmbientFighterEntity> {
    ...
    public LWSagasEffectsRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new FighterModel());
        this.shadowRadius = 0.4f;

    }

    @Override
    public RenderType getRenderType(AmbientFighterEntity animatable, ResourceLocation texture, @Nullable MultiBufferSource bufferSource, float partialTick) {
        return RenderType.entityCutoutNoCull(texture);
    }

    @Override
    public void render(AmbientFighterEntity entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        poseStack.pushPose();
    ...
    public void drawEffects(AmbientFighterEntity entity, Matrix4f baseMatrix, float partialTick, boolean showAura, boolean showLightning) {
        PoseStack poseStack = new PoseStack();
        poseStack.last().pose().set(baseMatrix);
        drawEffectsInline(entity, poseStack, partialTick, showAura, showLightning);
    }

    public void drawEffectsInline(AmbientFighterEntity entity, PoseStack poseStack, float partialTick, boolean showAura, boolean showLightning) {
    ...
    private void renderPulseAura(AmbientFighterEntity animatable, PoseStack poseStack, Minecraft mc, float partialTick) {
    ...
    private void drawSinglePulse(AmbientFighterEntity animatable, PoseStack poseStack, Minecraft mc, float partialTick, float progress) {
    ...
    private void executeAuraShaderDraw(AmbientFighterEntity animatable, PoseStack poseStack, Minecraft mc, float partialTick) {
        ShaderInstance shader = DMZShaders.auraShader;
        if (shader == null) return;
    ...
    private void executeLightningShaderDraw(AmbientFighterEntity animatable, PoseStack poseStack, float partialTick) {
