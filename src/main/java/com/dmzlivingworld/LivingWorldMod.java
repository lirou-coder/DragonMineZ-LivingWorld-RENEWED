...
public final class LivingWorldMod {
    ...
    public LivingWorldMod() {
        ...
        ModLoadingContext.get().registerConfig(
                ...
                LivingWorldConfig.SPEC,
                MOD_ID + "-server.toml"
        );

        ModLoadingContext.get().registerConfig(
                ModConfig.Type.CLIENT,
                LivingWorldClientConfig.SPEC,
                MOD_ID + "-client.toml"
        );
    ...
    private void registerAttributes(EntityAttributeCreationEvent event) {
        event.put(
                LWEntities.AMBIENT_FIGHTER.get(),
                AmbientFighterEntity.createAttributes().build()
        );

        event.put(
                LWEntities.WORLD_MENACE_FIGHTER.get(),
                AmbientFighterEntity.createAttributes().build()
        );
    }
}